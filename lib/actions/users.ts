...
async function validateAssignedLocations(
  ...
  if (locations.length !== locationIds.length) {
    throw new Error("Select only active locations for this user.");
  }
...
function buildLocationAssignments(locationIds: string[], defaultLocationId: string) {
...
export async function createUserAction(input: UserInput): Promise<ActionResult> {
  const actor = await getActionActorByPermission("users:create");
  ...
  const defaultLocationId = parsed.data.defaultLocationId;
  ...
  try {
    ...
    const createdUserName = await prisma.$transaction(async (tx) => {
      ...
      const locations = await validateAssignedLocations(tx, locationIds);
      const selectedPermissionKeys = hasRoleTables
        ? await tx.role
            .findFirst({
            ...
            .then((selectedRole) => {
              if (!selectedRole) {
                throw new Error("Select an active role for this user.");
              }

              return selectedRole.permissions.map((row) => row.permission.key);
            })
      ...
      const canUseMultipleLocations =
        selectedPermissionKeys.includes("location:view-all") ||
        selectedPermissionKeys.includes("location:multi");

      if (!canUseMultipleLocations && locationIds.length > 1) {
        throw new Error("This role can only be assigned to one preferred location.");
      }

      const defaultLocation = locations.find((loc) => loc.id === defaultLocationId);

      if (!defaultLocation) {
        throw new Error("Default location must be assigned to the user.");
      }
      ...
      const user = await tx.user.create({
        data: {
          ...
          locationAssignments: {
            create: buildLocationAssignments(locationIds, defaultLocation.id),
          },
        },
        select: {
          id: true,
...
export async function updateUserAction(
  ...
  const actor = await getActionActorByPermission("users:edit");
  ...
  try {
    ...
    const updatedUserName = await prisma.$transaction(async (tx) => {
      ...
      for (const assignment of buildLocationAssignments(locationIds, defaultLocation.id)) {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      await createAuditLog(tx, {
        ...
        entityType: "User",
        entityId: user.id,
        ...(user.defaultLocationId ? { locationId: user.defaultLocationId } : {}),
        before: {
          isActive: user.isActive,
        },
        after: {
...
export async function deleteUserAction(
  ...
  const actor = await getActionActorByPermission("users:delete");
  ...
  try {
    const deletedUserName = await prisma.$transaction(async (tx) => {
      ...
      if (user.role === "ADMIN") {
        ...
        return { name: displayName, wasDeactivated: true };
      }
      ...
      return { name: displayName, wasDeactivated: false };
    });
    ...
    return {
      success: true,
      message: deletedUserName.wasDeactivated
        ? `${deletedUserName.name} was deactivated (Admin accounts are deactivated instead of deleted).`
        : `${deletedUserName.name} was deleted. Past activity has been kept in history.`,
    };
  } catch (error) {
    return {
      success: false,
