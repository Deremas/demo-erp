...
export function getSimpleColumnSizing(column: SimpleColumn) {
  ...
  const isLocationText = key.includes("location");
  const isCompactText =
    key.includes("code") ||
    key.includes("role") ||
    key.includes("type") ||
  ...
  if (column.type === "multiline") {
    ...
    return ensureHeaderFits(column.header, {
      size: isDetailList ? 380 : 280,
      minSize: isDetailList ? 320 : 220,
      maxSize: isDetailList ? 520 : 360,
    });
  }
...
export const materialTableSearchTextFieldProps = {
  ...
  sx: {
    ...
    "& .MuiOutlinedInput-root": {
      ...
      "&:hover fieldset": {
        borderColor: "hsl(var(--brand-cyan) / 0.55)",
      },
      "&.Mui-focused fieldset": {
        borderColor: "hsl(var(--brand-cyan))",
      },
    },
