...
function getAuthBaseUrl() {
  ...
  const config: DynamicAuthBaseUrl = {
    allowedHosts: Array.from(allowedHosts),
    protocol: "https",
  };

  if (process.env.VERCEL_URL) {
    config.fallback = `https://${process.env.VERCEL_URL}`;
  }
...
export const auth = betterAuth({
  ...
  trustedOrigins: [
    ...
    ...(process.env.BETTER_AUTH_ALLOWED_ORIGINS?.split(",").map((origin) => origin.trim()).filter(Boolean) ?? []),
  ],
  secret:
