...
import { buildItemsListFileName, buildItemsListPdfBuffer, type ItemsListRow } from "@/lib/items-list-export";
...
export async function GET() {
  ...
  const pdf = buildItemsListPdfBuffer(rows);

  await prisma.auditLog.create({
    data: {
      actorUserId: user.id,
      action: "ITEMS_LIST_PDF_EXPORTED",
      entityType: "Report",
      entityId: "items-list",
  ...
  return new NextResponse(new Uint8Array(pdf), {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="${buildItemsListFileName("pdf")}"`,
