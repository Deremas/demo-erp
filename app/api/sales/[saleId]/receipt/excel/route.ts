...
import { buildReceiptExcelBuffer, buildReceiptFileName } from "@/lib/sales-receipt-export";
...
export async function GET(_: Request, context: { params: Promise<{ saleId: string }> }) {
  ...
  const buffer = buildReceiptExcelBuffer(sale, companySettings, printedAt);

  await prisma.auditLog.create({
    data: {
      actorUserId: user.id,
      action: "SALE_RECEIPT_EXCEL_EXPORTED",
      entityType: "Sale",
      entityId: sale.id,
  ...
  const fileName = buildReceiptFileName(sale.saleNumber, sale.soldAt, "xlsx");
