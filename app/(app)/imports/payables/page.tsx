...
type ImportPayablesPageProps = {
  searchParams?: Promise<RouteSearchParams>;
};

export default async function Page({ searchParams }: ImportPayablesPageProps) {
  const params = await searchParams;
  const filters = tableFiltersFromSearchParams(params);

  return (
    <TablePage
      config={await getTablePageConfig("purchasesImportPayables", {
        ...filters,
        paymentStatus: "DUE",
      })}
    />
